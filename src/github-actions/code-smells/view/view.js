...
export class View {
    ...
    */
    ...
    generarFigurasAleatorias() {
        let numFiguras = prompt("Introduce el número de figuras que deseas crear: ") || "0";
        let numFigurasInt = parseInt(numFiguras);
    ...
    */
    crearFiguras(numFiguras) {
        ...
        for (let i = 0; i < numFiguras; i++) {
            ...
            switch (type) {
                case "cuadrado":
                    const side = this.generarSize();
                    this.figuras.push(new Cuadrado(x, y, side, color));
                    break;
                case "rectangulo":
                    const rectBase = this.generarSize();
                    const rectAltura = this.generarSize();
                    this.figuras.push(new Rectangulo(x, y, rectBase, rectAltura, color));
                    break;
                case "triangulo":
                    const triBase = this.generarSize();
                    const triAltura = this.generarSize();
                    this.figuras.push(new Triangulo(x, y, triBase, triAltura, color));
                    break;
                case "circulo":
                    const radius = this.generarSize();
